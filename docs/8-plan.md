...
- 완료 조건
  ...
  - [ ] 관리자가 **타인 글의** 상태를 `DONE`으로 변경하면 200으로 성공함
...
- 완료 조건
  - [ ] 한 회원이 여러 WBS에 같은 날짜로 시간을 넣은 경우 `total_hours`가 합산되어 반환됨
  - [ ] 비로그인 요청 시 401 반환
  - [ ] 타인 `user_id`로도 조회가 성공함
...
- 완료 조건 (서버 무관)
  - [ ] 상태값/색상 상수가 `constants.ts`에만 정의됨 — `rg "'TODO'|'IN_PROGRESS'|'QA'|'RESOLVED'|'DONE'" frontend/src --glob '!**/constants.ts'` 결과가 0건
  - [ ] `wbsPanelStore`가 정의되어 임의 컴포넌트에서 패널을 열고 닫을 수 있음
- 완료 조건 (BE-1 이후 연동 확인)
  - [ ] `JWT_ACCESS_SECRET` 만료시간을 임시로 10초로 낮춘 상태에서, 만료 후 API 호출이 자동 재발급되어 화면 조작 없이 이어짐
  - [ ] 재발급 실패 시 로그인 화면으로 이동함
  - [ ] 로그인 후 새로고침하면 로딩 화면을 거쳐 캘린더가 유지되고, 쿠키를 삭제한 뒤 새로고침하면 로그인 화면으로 이동함
...
- 완료 조건
  ...
  - [ ] 로그인/회원가입 실패 시 서버 `error.message`가 필드 하단에 표시됨
  - [ ] 회원가입 후 해당 계정으로 로그인이 가능함
  - [ ] 로그아웃 후 다른 계정으로 로그인했을 때 이전 계정의 WBS가 한순간도 보이지 않음(캐시 초기화 확인)
...
- 완료 조건
  ...
  - [ ] 이전주/다음주 이동 시 해당 기간의 WBS로 갱신됨
  - [ ] 375px 폭에서 그리드가 가로 스크롤로 열람 가능함
  - [ ] 막대 클릭 시 `wbsPanelStore`의 대상 id가 갱신됨
...
- 완료 조건
  ...
  - [ ] 막대 끝단 드래그로 기간을 조정하면 서버에 반영되고, 목록 쿼리 invalidate로 캘린더가 갱신됨
  - [ ] 3절 전환 트리거 검토 결과가 기록됨(원안 유지 또는 축소판 전환)
...
- 완료 조건
  - [ ] 캘린더에서 막대를 클릭하면 저장된 입력값과 시간 할당이 그대로 표시됨
  - [ ] 일자별 시간 입력 시 이 WBS의 합계가 자동 표시됨
  - [ ] 그 담당자의 해당 날짜 총합(저장분 + 미저장 입력값)이 8시간을 넘는 날에만 그 날짜 옆에 경고가 표시되고, 저장은 차단되지 않음
  - [ ] 담당자/작성자가 탈퇴 회원이면 이름 옆에 "(탈퇴)"가 표시됨
  - [ ] 일반 회원 화면에서 `DONE` 옵션이 선택 불가함
  - [ ] 삭제 버튼이 작성자 본인에게만 노출됨(관리자 포함 타인에게는 미노출)
  - [ ] 종료일 < 시작일 입력 시 저장이 차단되고 오류 메시지가 표시됨
  - [ ] 저장 후 캘린더 막대가 즉시 갱신됨(쿼리 invalidate 확인)
  - [ ] 375px 폭에서 우측 패널이 하단 시트로 전환됨
...
- 완료 조건
  ...
  - [ ] 캘린더 5주 범위를 벗어난 과거 WBS도 목록에 표시됨
  - [ ] 각 상태의 건수가 표시됨
  - [ ] 목록 항목 클릭 시 상세패널이 열림(FE-4 컴포넌트 재사용)
  - [ ] 375px 폭에서 하단 시트 또는 전체폭 오버레이로 전환됨
