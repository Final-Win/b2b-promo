...
- 작업
  ...
  - 발견 버그 수정
- 완료 조건
  - [x] 테스트 4종이 각 1개 이상 작성되고 전부 통과함 — `backend/tests/*.test.js` 4개 파일, 54개 테스트 전부 통과(커버리지 94.56%). ①권한검증 `wbs-admin.test.js`, ②상태전이 `wbs-admin.test.js`, ③8h경고 `time-allocations.test.js`, ④work_date범위 `wbs.test.js`
  - [x] 일반 회원 시나리오 1-1~1-8이 각 시나리오의 완료 조건대로 수행됨 — `backend/scripts/e2e-scenario.js`로 API 레벨 21개 항목 전부 통과 + 브라우저(Playwright)로 로그인/캘린더/드래그등록/상세패널/내 WBS관리/로그아웃 흐름 재확인
  - [x] 관리자 시나리오 2-1(타인 글 수정), 2-2(타인 글 DONE 처리)가 수행됨 — e2e-scenario.js + 브라우저에서 관리자 계정으로 타인 글 열람→DONE 저장→캘린더 즉시 갱신 확인(검증 후 원래 상태로 복원)
  - [x] DB-2 시드(회원 4명 / WBS 200건) 상태에서 캘린더 5주 조회가 1초 이내에 응답함 — `seed.sql`을 임시로 200건으로 늘려 측정, 평균 19.8ms(5회), 검증 후 1건으로 되돌림
  - [x] 375px 폭에서 캘린더 메인·상세패널(하단 시트 전환)·내 WBS관리 탭이 정상 동작함 — 캘린더는 가로 스크롤 정상. **상세패널/내 WBS관리 탭에서 버그 발견 후 수정**(아래 참고)
  - [x] 위 검증에서 발견된 버그가 모두 수정되었거나, 미수정 건이 잔여 이슈로 기록됨 — 아래 "QA-1 발견 버그" 참고, 전부 수정 완료

### QA-1 발견 버그
- **375px 미디어쿼리 불일치**: `WbsDetailPanel.css`/`MyWbsTabs.css`의 `@media (max-width: 375px)`가 정확히 375px 뷰포트에서 `devicePixelRatio`(예: `1.0000000149...`) 부동소수점 오차로 매칭에 실패해 하단 시트로 전환되지 않음. `max-width: 480px`로 여유를 두어 수정.
