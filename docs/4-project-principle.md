...
- Refresh Token은 httpOnly 쿠키로 저장(서버가 `Set-Cookie`로 내려줌, 프론트 JS는 접근하지 않음). Access Token만 메모리(Zustand)에 두며, 새로고침 시 쿠키 기반으로 재발급받아 로그인 상태를 복원(시나리오 1-8 "세션 유지").
...
- 회원별 일자 시간 합계(8시간 초과 경고)는 서버가 계산해서 반환하고, 프론트는 받은 값을 표시만 한다. 전용 조회 `GET /time-allocations/daily-sum?userId&date`를 두고, 저장 자체는 WBS 등록/수정 요청 바디에 `timeAllocations` 배열을 포함해 한 번에 처리(별도 등록 API 불필요).
...
- 환경변수(`DB_URL`, `JWT_ACCESS_SECRET`, `JWT_REFRESH_SECRET`, `PORT` 등)는 `.env` + `.env.example`로 관리, 저장소에 실제 값 커밋 금지.
...
- 캘린더 조회(1초 이내, PRD 7절) 대응 인덱스 2개만: `wbs(start_date, end_date)`, `time_allocations(wbs_id, work_date)`.
...
│   │   ├── client.ts                # fetch 래퍼: 401 시 refresh 재시도 1회
│   │   ├── authApi.ts               # useLogin, useSignup, useLogout, authStore(Zustand: accessToken)
│   │   ├── wbsApi.ts                # useWbsList, useCreateWbs(시간할당 포함), useUpdateWbs, useDeleteWbs
│   │   ├── timeAllocationApi.ts     # useDailySum (8시간 초과 경고용 조회)
│   │   └── queryClient.ts
│   ├── shared/                  # 공통 UI/유틸 (상위 계층 import 금지)
│   │   ├── components/             # Button, Modal 등
│   │   └── constants.ts             # 상태값 enum, 색상 매핑
│   ├── App.tsx                   # 라우터 설정
│   └── main.tsx
└── package.json
```
